...
def find_last_checkpoint(model_file):
    ...
    if os.path.exists(base_directory):
        state_files = []
        files = [os.path.basename(f) for f in os.listdir(base_directory) if
        ...
        if len(state_files) > 0:
            epochs = [state_files[i].replace(file_name, '').replace('.pth.tar', '').replace('.', '') for i in
                      range(len(state_files))]
        ...
        for epoch in range(self.start_epoch, epochs + 1):
            ...
            for i, data in enumerate(train_dataloader, start=0):
                ...
                if i == len(train_dataloader) - 2:
                    ...
                    self.writer.add_images('train/adversarial_images_cl',
                                           torch.clamp(images + perturbation, self.box_min, self.box_max),
                                           global_step=epoch)
